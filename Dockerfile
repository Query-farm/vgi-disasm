...
#
# Single image that serves every network transport of the `disasm` VGI worker:
...
#
# This worker is STATELESS: it is pure static decoding over the input BLOB (the
# Capstone engine + ATT&CK tables are compiled into the binary), with no
# per-attach state, no registry, and no network egress. So there is no /data
...
# runtime ships. The workspace uses only published crates (vgi/vgi-rpc/arrow +
# capstone/goblin), so no network fetch beyond crates.io is needed.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin disasm-worker \
    && cp target/release/disasm-worker /disasm-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-disasm
...
LABEL org.opencontainers.image.title="vgi-disasm" \
      org.opencontainers.image.description="Disassemble PE/ELF/Mach-O binaries and shellcode into instruction rows with section/import/string/capability triage, as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_DISASM_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /disasm-worker /usr/local/bin/disasm-worker
