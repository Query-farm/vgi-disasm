...
#
# Dispatch the single vgi-disasm image into one of its transports:
...
#
# The worker is stateless (pure static decode of the input BLOB), so there is no
...
case "${1:-http}" in
  http)
    ...
    exec disasm-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec disasm-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec disasm-worker "$@"
