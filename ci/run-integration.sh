...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-disasm
...
# is what LOCATION (VGI_DISASM_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_DISASM_WORKER = the stdio worker command (DuckDB spawns it).
#   http        start `disasm-worker --http` (auto port; advertises `PORT:<n>`),
#               VGI_DISASM_WORKER = http://127.0.0.1:<port>.
#   unix        start `disasm-worker --unix <sock>` (advertises `UNIX:<sock>`),
#               VGI_DISASM_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
#   WORKER_BIN        path to the compiled disasm-worker binary.
# Optional:
...
# Create STAGE up front: the http/unix worker log is written here *before* the
# test tree is staged, so the directory must exist even when STAGE is passed in.
mkdir -p "$STAGE"

WORKER_BIN="${WORKER_BIN:-$REPO/target/release/disasm-worker}"
...
# Bring up the out-of-band server for http/unix and resolve VGI_DISASM_WORKER.
start_server_and_set_location() {
  local kind="$1"
  : "${WORKER_BIN:?path to the disasm-worker binary (WORKER_BIN)}"
  ...
  case "$kind" in
    http)
      ...
      export VGI_DISASM_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_DISASM_SOCK:-/tmp/disasm.$$.sock}"
      ...
      export VGI_DISASM_WORKER="unix://$SOCK_PATH"
...
case "$TRANSPORT" in
  subprocess)
    export VGI_DISASM_WORKER="${VGI_DISASM_WORKER:-$WORKER_BIN}"
    ;;
  http)
    if [[ "${VGI_DISASM_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_DISASM_WORKER"
    else
...
: "${VGI_DISASM_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"
...
# Copy fixture data referenced by path-based tests.
if [[ -d "$REPO/test/sql/data" ]]; then
  mkdir -p "$STAGE/test/sql/data"
  cp -R "$REPO/test/sql/data/." "$STAGE/test/sql/data/" 2>/dev/null || true
fi
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_DISASM_WORKER, pattern: $TEST_PATTERN) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_DISASM_WORKER." >&2
  exit 1
fi
