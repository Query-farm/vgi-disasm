...
# default `ignore_error_messages`, so a missing httpfs looks like a pass-by-skip
# rather than a failure). We therefore inject a signed
# `INSTALL httpfs FROM core; LOAD httpfs;` right after the test's own `LOAD vgi;`
# so the http leg actually runs. Harmless/absent for subprocess and unix.
/^require[ \t]+vgi[ \t]*$/ {
...
# The vgi-disasm .test files LOAD vgi explicitly (statement ok + LOAD vgi;)
