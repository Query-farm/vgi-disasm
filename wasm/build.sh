#!/usr/bin/env bash
# Build the browser (`worker:`) build of the disasm VGI worker: disasm-wasm
# compiled to wasm32-unknown-emscripten, linked by emcc into a MODULARIZE'd Web
# Worker module served over a SharedArrayBuffer byte channel.
#
...
#   - a nightly toolchain with the rust-src component: -Z build-std is required
#     because the pthread build recompiles compiler_builtins with atomics.
#   - the wasm32-unknown-emscripten target for that toolchain.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# and without them wasm-ld rejects --shared-memory.
...
LIB="target/$TARGET/release/libdisasm_wasm.a"
...
# threads convoy on it. mimalloc is thread-caching and scales about as well as
# native. Override with MALLOC=dlmalloc to compare.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten's 64 KiB default stack is
# too small — the Arrow encoders and decode path nest deeply, and an overflow
# surfaces as a bare "memory access out of bounds" with no stack trace. The serve
# threads run the actual work, so the pthread stack matters most. INITIAL_MEMORY
