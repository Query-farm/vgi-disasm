...
//! the `disasm-wasm` crate (the browser build, which serves the same `Worker`
...
/// defaulting `VGI_WORKER_CATALOG_NAME` to `disasm` when unset.
...
/// Build the fully-registered `disasm` VGI worker, shared verbatim by the native
/// binary (`main.rs`) and the browser (`disasm-wasm`) entrypoints. It registers
/// every scalar / table function and sets the catalog metadata, but does **not**
/// start a transport — the caller picks `run()` (native) or the wasm serve loop.
